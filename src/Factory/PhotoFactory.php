...
class PhotoFactory
{
     ...
     */
    public function createFromLinuxFiles(array $linux_files, SynologyPhotoCollection $synology_photo_collection): void
    {
...
// TODO : Do we need this - should it be handled in ExifDataFactory::ThrowExceptionIfValidationFails method ?
        ...
        try {
            $this->photo_repository->bulkAdd($photo_list_to_bulk_insert);
        } catch (Exception $exception) {
            throw new PhotoCentralSynologyServerException("Cannot bulk add photos. {$exception->getMessage()}");
        }
        try {
            ...
            throw new PhotoCentralSynologyServerException("Cannot bulk update imported flag on linux files {$exception->getMessage()}");
        }
    }
}
